...
export const protect = asyncHandler(async (req, res, next) => {
  ...
  try {
    ...
    if (req.isAllBrands && req.user.role !== 'superadmin') {
      res.status(403);
      throw new Error('Forbidden: Only superadmins can access ALL brands');
    }

    if (req.brandId && req.user.role !== 'superadmin' && req.user.role !== 'customer') {
      const userBrands = req.user.brandIds ? req.user.brandIds.map(id => id.toString()) : [];
      if (!userBrands.includes(req.brandId.toString())) {
        res.status(403);
        throw new Error('Forbidden: You do not have access to this brand');
      }
    }
