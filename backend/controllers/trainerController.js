...
export const createTrainer = asyncHandler(async (req, res) => {
  const { name, bio, specialties, phone, email, avatarUrl, gallery, status, locationIds: providedLocationIds, password, compensationType, compensationRate } = req.body;
  ...
  const created = await Trainer.create({
    name, bio, specialties, phone, email, avatarUrl, gallery, status, locationIds, userId, compensationType, compensationRate
  });
